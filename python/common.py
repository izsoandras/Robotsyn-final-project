...
def project(K, X, distCoeffs=[0,]*5):
    """
    ...
    dy = (k1 * r ** 2 + k2 * r ** 4 + k3 * r ** 6) * y + p1 * (r ** 2 + 2 * y ** 2) + 2 * p2 * x * y

    x = x+dx
    y = y+dy

    xy = np.vstack((x,y, np.ones_like(x)))
    xy = np.reshape(xy, [xy.shape[0],-1]) # Needed to support N=1
    uvw = K@xy[:3,:]
    uvw /= uvw[2,:]
    return uvw[:2,:]
